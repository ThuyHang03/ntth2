...
import HomeBar from './HomeBar';//gọi thanh taskbar
    ...
    const handlePayment = async () => {
      try {
        if (!hasAddedCard) {
            Alert.alert(
                'Thêm thẻ',
                'Vui lòng thêm thẻ trước khi thanh toán.'
              );
              return;
      }
        const totalAmount = calculateTotalPrice();//thông báo
        // Xóa tất cả sản phẩm trong giỏ hàng
        await AsyncStorage.removeItem('cart');
        setCartItems([]); // Cập nhật state để làm mới giao diện
        // Hiển thị thông báo thanh toán thành công hoặc chuyển hướng đến màn hình thanh toán thành công
        Alert.alert(
    ...
    return (
      <>
        <View style={styles.container}>
            ...
            <TouchableOpacity style={styles.paymentButton} onPress={handleAddCard}>
